...
setwd("~/Desktop/Computational Models/P2")
...
ricplot = ggplot(riccardo,aes(grid,posterior))+ 
  geom_point()+
  geom_line()+
  theme_classic()+ 
...
kriplot = ggplot(kristian,aes(grid,posterior))+ 
  geom_point()+
  geom_line()+
  theme_classic()+ 
...
josplot =ggplot(josh,aes(grid,posterior))+ 
  geom_point()+
  geom_line()+
  theme_classic()+ 
...
mikplot = ggplot(mikkel,aes(grid,posterior))+ 
  geom_point()+
  geom_line()+
  theme_classic()+ 
...
#MAP
riccardo$grid[ which.max(riccardo$posterior)] #.50
#Median
median(RiccardoSamples) #0.5
...
#MAP
kristian$grid[ which.max(kristian$posterior)] #1
#calculating value which minimizes loss
#lossf(kristian) #0.5
#Median
median(KristianSamples) #0.79
...
#MAP
josh$grid[ which.max(josh$posterior)] #.81
#calculating value which minimizes loss
median(JoshSamples) #0.81
...
#MAP
mikkel$grid[ which.max(mikkel$posterior)] #.5
#Median
median(RiccardoSamples) #0.5
...
The 'best' teacher if comparing only by MAP value is Kristian, with a value of 1. However there is a lot of uncertainty surrounding his estimate due to the low sample size. If the median is chosen as the criteria, Josh actually scores slightly higher than Kristian (0.81 vs 0.79) and wins the title of best teacher
